...
// In the vagrant box you need to install clang-tools
 ...
 */
 ...
 [_QWERTY] = LAYOUT( \
  ...
  ST_MACRO_GRAVE,   KC_A,   KC_S,    KC_D,    KC_F,    KC_G,                     KC_H,    KC_J,    KC_K,    KC_L,    KC_SCLN, KC_QUOT, \
  KC_LSFT,  KC_Z,   KC_X,    KC_C,    KC_V,    KC_B, KC_EQL , KC_MINS,  KC_N,    KC_M,    KC_COMM, KC_DOT,  KC_SLSH,  KC_RSFT, \
                             OSM(MOD_RGUI), KC_BSLASH, MO(_NAV), KC_ENT,   KC_SPC,   MO(_VIMIDEA),   OSM(MOD_RCTL), OSM(MOD_RALT) \
),

  // vim normal
 [_NAV] = LAYOUT( \
  KC_NO      ,  KC_NO    ,   KC_NO       , KC_NO         , KC_NO     , KC_NO                     , KC_HOME          , KC_DOWN         , KC_PGUP        , KC_END          , KC_NO          ,  KC_NO  ,
  TO(_QWERTY), KC_NO     , LCTL(KC_RIGHT), LCTL(KC_RIGHT), LCTL(KC_Y), KC_NO                     , LCTL(KC_INSERT), LCTL(KC_Z)    , KC_HOME      , KC_END         , LSFT(KC_INSERT), KC_PGUP ,
  KC_NO    , TO(_QWERTY) , KC_NO         , KC_DELETE    , KC_NO     , KC_NO                          , KC_LEFT        , KC_DOWN       , KC_UP        , KC_RIGHT       , LALT(KC_DOWN)  , KC_PGDN   ,
  KC_LSHIFT    , KC_NO     , LCTL(KC_X)     , KC_DELETE     , KC_NO     , LCTL(KC_LEFT), KC_LBRC, KC_RBRC, KC_F3          , KC_APPLICATION, LSFT(KC_TAB), KC_TAB , LCTL(KC_F)     , KC_RSHIFT   ,
                             KC_LALT, KC_LCTRL, MO(_NAV), KC_ENT,   KC_F23 /*autocomplete*/,   MO(_APP),   OSM(MOD_RCTL), OSM(MOD_LALT) \
  ),
  ...
  [_APP] = LAYOUT(
    ...
    LCTL(KC_F4)                , LALT(LCTL(LSFT(KC_Q))) , LALT(LCTL(LSFT(KC_W))) , LALT(LCTL(LSFT(KC_E))) , LALT(LCTL(LSFT(KC_R))) , LCTL(LGUI(KC_5))                                                         , LALT(LCTL(LSFT(KC_Y))) , LCTL(LGUI(KC_8))       , LCTL(LGUI(KC_1))       , LCTL(LGUI(KC_5))       , LALT(LCTL(LSFT(KC_P))) , LCTL(KC_F4)    ,
    LGUI(KC_TAB)               , LGUI(KC_A)             , LCTL(LGUI(KC_8))       , LALT(LCTL(LSFT(KC_D))) , LCTL(LGUI(KC_7))       , LALT(LCTL(LSFT(KC_G)))                                                   , LCTL(LGUI(KC_4))       , LALT(LCTL(LSFT(KC_J))) , LCTL(LGUI(KC_3))       , LALT(LCTL(LSFT(KC_L))) , LCTL(LGUI(KC_2))       , LGUI(KC_UP)    ,
    LALT(KC_ESCAPE)            , LALT(LCTL(LSFT(KC_Z))) , LCTL(LGUI(KC_9))       , LALT(LCTL(LSFT(KC_C))) , LALT(LCTL(LSFT(KC_V))) , LALT(LCTL(LSFT(KC_B))) , LALT(LCTL(KC_PSCREEN))       , KC_PSCREEN             , LALT(LCTL(LSFT(KC_N))) , LCTL(LGUI(KC_6))       , LALT(KC_TAB)           , LALT(KC_GRAVE)         , LCTL(LGUI(KC_0))       , LGUI(KC_DOWN)  ,
                                                                                         LALT(KC_MINUS)         , LALT(KC_EQUAL)         , KC_TRANSPARENT         , LALT(KC_F4)            , KC_NO                  , KC_TRANSPARENT         , KC_AUDIO_VOL_UP        , KC_AUDIO_VOL_DOWN
  )
 ...
 */
  ...
  /*   KC_LSHIFT                        , KC_NO      , KC_NO      , ST_MACRO_10 , ST_MACRO_11 , ST_MACRO_31 , KC_NO     , KC_NO   , KC_NO         , KC_NO         , ST_MACRO_LABK , ST_MACRO_RABK , KC_BSLASH , KC_NO , */
  /*                            KC_LALT , KC_LCTRL   , MO(_APP)      , MO(_NAV)    , KC_SPC      , MO(_APP)       , KC_RCTRL  , KC_RALT */
  /* ), */
  ...
  /*   KC_LSHIFT                        , LCTL(KC_B) , KC_NO   , KC_NO    , KC_NO   , ST_MACRO_31 , KC_NO    , KC_NO    , KC_NO         , KC_NO         , KC_NO         , KC_NO    , KC_NO , KC_RSHIFT , */
  /*                            KC_LALT , KC_LCTRL   , MO(_APP)   , MO(_NAV) , KC_SPC  , MO(_APP)       , KC_RCTRL , KC_RALT */
  /*  ) */
};
